...
def get_recommended_jobs(profile, limit=10):
    profile_text = profile_to_text(profile)
    results = recommend_jobs(profile_text, limit=limit)

    job_ids = [p.payload["job_id"] for p in results if p.payload and "job_id" in p.payload]
    if not job_ids:
        return Job.objects.none()

    job_scores = {p.payload["job_id"]: p.score for p in results if p.payload and "job_id" in p.payload}
    jobs = list(Job.objects.select_related("company").filter(id__in=job_ids))
