...
from accounts.models import Profile, Job, Application, Resume,CompanyProfile
...
def jobseeker_dashboard(request):
    ...
    jobs = Job.objects.filter(status="Open").exclude(id__in=applied_job_ids).order_by("-created_at")[:3]
    ...
    if has_skills:
        try:
            recommended_jobs = get_recommended_jobs(profile, applied_job_ids=applied_job_ids)
